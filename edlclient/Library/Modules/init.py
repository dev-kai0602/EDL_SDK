...
class modules(metaclass=LogBase):
    ...
    def generic(self):
        try:
            return generic(fh=self.fh, serial=self.serial, args=self.args, loglevel=self.__logger.log_level)
    ...
    def ops(self):
        try:
            return oneplus(fh=self.fh, projid=self.devicemodel, serial=self.serial, loglevel=self.__logger.log_level,
